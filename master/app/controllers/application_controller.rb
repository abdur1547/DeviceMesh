class ApplicationController < ActionController::API
  def success_json_response(data:, status:, serializer: nil)
    return render json: {success: true, status: status, data: ActiveModelSerializers::SerializableResource.new(data, {serializer: serializer})}, status: status if serializer
